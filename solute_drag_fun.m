function ff=solute_drag_fun(x,Temp,C0_mn,wC_A,wC_F,Ux, ...
...
Mufe_gamma_int=Para_Fe(1,1)+R.*Temp.*((xFe_A_int-1)-1/2*(xFe_A_int-1)^2)+R.*Temp.*Para_Fe(1,2); % series expansion
% Muc_gamma_int=Para_C(1,1)+R.*Temp.*log(x(1))+R.*Temp.*(Para_C(1,2).*x(1)+Para_C(1,3).*xMn_A_int);
Muc_gamma_int=Para_C(1,1)+R.*Temp.*((x(1)-1)-1/2*(x(1)-1)^2)+R.*Temp.*(Para_C(1,2).*x(1)+Para_C(1,3).*xMn_A_int); % series expansion
...
%%%%%%%%%%%%%%%%% with no overlapp of diffusion field
ff(1)=30*Vbcc*(X0-Xneq)/(4*pi*(x(1)-X0))*1/SN-(x(2)^3+5*Rbcc*x(2)^2+10*Rbcc^2*x(2))*1/SN;
ff(2)=(x(1)-Xneq)*x(2)-Z*(x(1)-X0);
